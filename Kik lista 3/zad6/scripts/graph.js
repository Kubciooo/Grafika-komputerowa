...
const Graph = (canvas) => {
    gl = canvas.getContext('webgl', { stencil: false });
    resize(canvas, gl);
    let points = [[],[]];
    let normals = [[],[]];
    ...
    const maskDraw = (worldView, worldView2, projection, colors) => {
        const views = [worldView, worldView2];
        gl.clearColor(0,0,0,1);
        gl.clear( gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT );
        gl.colorMask(true, false, false, true);
        draw(0, views[0], projection, colors[0]);
        gl.clear( gl.DEPTH_BUFFER_BIT ); // clear only the depth buffer
        gl.colorMask(false, true, true, true);
        draw(0, views[1], projection, colors[1]);
      }
...
window.addEventListener('load', () => {
    ...
    createGraph((x, y) => x ** 2 + y ** 2, ((x,y) => -1*x**2 - y**2 + 1));
});
...
const createGraph = (func1, func2, xBounds = { min: -1, max: 1 }, yBounds = { min: -1, max: 1 }) => {
    ...
    const drawFunction = () => {
        ...
        let cameraMatrixTranslated = M4.translate(cameraMatrix, 10, 0, 0);
        graph.maskDraw(M4.inverse(cameraMatrix), M4.inverse(cameraMatrixTranslated), projectionMatrix, colors);
    }
    const drawLoop = () => {
        if(hasChanged) {
            drawFunction();
